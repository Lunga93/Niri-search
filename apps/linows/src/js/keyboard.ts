...
import * as qactions from './components/qactions.ts';
import * as actionmenu from './components/actionmenu.ts';
import * as rowactions from './components/rowactions.ts';
...
import { canRunElevated } from './platform.ts';
import { trash as trashIcon } from './icons.ts';
import { classifyResultId, isSyntheticResultId } from './catalog.ts';
...
function handleKeyDown(e) {
    ...
    if (e.altKey && (e.shiftKey || shiftHeld) && (e.key === 'Q' || e.key === 'q')) {
        e.preventDefault();
        import('./ipc.ts').then((m) => m.quitApp());
        return;
    }
